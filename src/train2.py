...
def train_evaluate(df_train, df_val, src_vocab, tgt_vocab, device="cpu"):
    ...
    encoder = Encoder(len(src_vocab), emb_dim=config.embedding_dim, hid_dim=config.hidden_dim, embeddings=src_vocab)
    decoder = Decoder(len(tgt_vocab), emb_dim=config.embedding_dim, hid_dim=config.hidden_dim, embeddings=tgt_vocab)
